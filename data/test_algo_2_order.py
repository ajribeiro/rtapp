...
qry = "SELECT actor_name,actor_id,imdb_id,billed_num FROM actors order by billed_num;"
actorlist = mydb.query(qry)
casts_df = pd.DataFrame(actorlist, \
    columns=['actor_name','actor_id','imdb_id','billed_num'])

qry = "SELECT actor_name,actor_id,ratio,total FROM actor_stats;"
actor_stats = mydb.query(qry)
actor_stats_df = pd.DataFrame(actor_stats, \
    columns=['actor_name','actor_id','ratio','total'])
...
for m in mlist:
    ...
    # actorlist = mydb.query(qry)
    actorlist = casts_df[casts_df.imdb_id == imdb_id].actor_name.tolist()
    billedlist = casts_df[casts_df.imdb_id == imdb_id].billed_num.tolist()
    actorsum = 0.
    totals = 1e-6
    try:
        for i in range(len(actorlist)):
            actor_name = actorlist[i]
            billed_num = billedlist[i]
            actor = actor_stats_df[actor_stats_df.actor_name == actor_name]
            ratio = actor.ratio.values[0]
            total = actor.total.values[0]
            totals += total+billed_num
            actorsum += ratio*(total+billed_num)
            actorsum /= totals
    except: 
        continue
